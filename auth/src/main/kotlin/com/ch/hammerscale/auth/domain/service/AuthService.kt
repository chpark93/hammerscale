...
import com.ch.hammerscale.auth.domain.model.user.UserRole
import com.ch.hammerscale.auth.domain.port.out.UserRepository
import com.ch.hammerscale.auth.infrastructure.security.JwtService
...
class AuthService(
    ...
    fun register(
        ...
        if (userRepository.existsByEmail(email = email)) throw IllegalArgumentException("Email already registered: $email")
    ...
    fun login(
        ...
        if (!passwordEncoder.matches(rawPassword, user.passwordHash)) throw IllegalArgumentException("Invalid email or password")
