package com.ch.hammerscale.admin.domain.service

import com.ch.hammerscale.admin.domain.model.abuse.*
import com.ch.hammerscale.admin.domain.port.out.AbuseEventRepository
import com.ch.hammerscale.admin.domain.port.out.BlockedClientStore
import com.ch.hammerscale.admin.domain.port.out.RateLimitStore
...
class AbuseDetectionService(
    ...
    @Value($$"${hammerscale.abuse.auto-block-after-violations:10}") private val autoBlockAfterViolations: Int,
    @Value($$"${hammerscale.abuse.auto-block-duration-minutes:60}") private val autoBlockDurationMinutes: Int
) {
    fun checkBlocked(
        clientId: String
    ): BlockCheckResult {
        if (!blockedClientStore.isBlocked(clientId = clientId)) return BlockCheckResult.Allowed
    ...
    fun checkRateLimit(
        ...
        return if (allowed) RateLimitCheckResult.Allowed(
            remaining = remaining
        ) else RateLimitCheckResult.Throttled(
            remaining = remaining
        )
    }
