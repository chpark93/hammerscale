package com.ch.hammerscale.admin.infrastructure.abuse

import com.ch.hammerscale.admin.domain.model.abuse.RateLimitConfig
...
class InMemoryRateLimitStore(
    ...
    override fun allowRequest(
        ...
        val window = windows.compute(windowKey) { _, old ->
    ...
    override fun getRemainingRequests(
        ...
        val slot = now / (windowSeconds * 1000)
        val windowKey = "$clientId:$slot"
        val window = windows[windowKey] ?: return config.requestsPerMinute

        return (config.requestsPerMinute - window.count.get()).coerceAtLeast(0)
    }
