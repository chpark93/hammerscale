...
import com.ch.hammerscale.admin.domain.model.abuse.AbuseEventType
import com.ch.hammerscale.admin.domain.port.out.ServiceLoadMetrics
import com.ch.hammerscale.admin.domain.service.AbuseDetectionService
...
class AbusePreventionFilter(
    ...
    override fun doFilterInternal(
        ...
        when (val blockResult = abuseDetectionService.checkBlocked(clientId = clientId)) {
            is AbuseDetectionService.BlockCheckResult.Blocked -> {
                ...
                sendErrorResponse(response, HttpStatus.FORBIDDEN.value(), "Access denied. Your client has been blocked.")
        ...
        when (val rateResult = abuseDetectionService.checkRateLimit(clientId = clientId)) {
            is AbuseDetectionService.RateLimitCheckResult.Throttled -> {
                ...
                sendErrorResponse(response, HttpStatus.TOO_MANY_REQUESTS.value(), "Rate limit exceeded. Please try again later.")
                serviceLoadMetrics.recordRequest(request.requestURI ?: "", System.currentTimeMillis() - startMs, 429)
    ...
    private fun resolveClientId(
        ...
        return if (forwarded.isNullOrBlank()) request.remoteAddr ?: "unknown"
        else forwarded.split(",").firstOrNull()?.trim() ?: request.remoteAddr ?: "unknown"
    }
    ...
    private fun sendErrorResponse(
        ...
        val body = ErrorResponseBody(
            message = message,
            error = ErrorResponseBody.ErrorDetail(
