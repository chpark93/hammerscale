package com.ch.hammerscale.control.infrastructure.persistence

import com.ch.hammerscale.control.domain.model.*
import com.ch.hammerscale.control.domain.port.out.TestPlanRepository
import com.ch.hammerscale.control.infrastructure.persistence.table.TestPlans
...
class TestPlanExposedAdapter(
    ...
    override fun save(
        ...
        val headersJson = if (testPlan.config.headers.isNotEmpty()) objectMapper.writeValueAsString(testPlan.config.headers) else null
        val queryParamsJson = if (testPlan.config.queryParams.isNotEmpty()) objectMapper.writeValueAsString(testPlan.config.queryParams) else null
    ...
    override fun findById(
        ...
        return TestPlans.selectAll().where {
            TestPlans.id eq id
    ...
    private fun ResultRow.toDomain(): TestPlan {
        ...
        val headers: Map<String, String> = if (headersJson != null) objectMapper.readValue(headersJson, object : TypeReference<Map<String, String>>() {}) else emptyMap()
        val queryParams: Map<String, String> = if (queryParamsJson != null) objectMapper.readValue(queryParamsJson, object : TypeReference<Map<String, String>>() {}) else emptyMap()
        val testType = TestType.valueOf(this[TestPlans.testType])
        val stressTestConfig = if (testType == TestType.STRESS) StressTestConfig(
        ...
        val spikeTestConfig = if (testType == TestType.SPIKE) SpikeTestConfig(
        ...
        return TestPlan(
            ...
            createdAt = LocalDateTime.ofInstant(this[TestPlans.createdAt], ZoneId.systemDefault())
        )
    }
}
